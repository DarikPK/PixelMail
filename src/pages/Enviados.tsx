...
const Enviados = () => {
  ...
  // Reutilizar el listener central de EmailContext. Antes esta pantalla abría un
  // segundo listener de Firestore sobre la misma colección.
  const emails = useMemo<SentEmailData[]>(() => {
    return allEmails
      .filter((email) => email.direction !== 'inbound' && !email.deleted)
      .map((email) => ({
        id: email.id,
        to: email.to,
        ...
        createdAt: email.createdAt || email.receivedAt || new Date(0),
  ...
  const handleDeleteSent = async (e: React.MouseEvent, emailId: string) => {
    ...
    try {
      ...
      console.error('Error al mover correo enviado a papelera:', error);
    }
  };
  ...
  const handleMoveAllToTrash = async () => {
    ...
    try {
      ...
      console.error('Error al mover todos los enviados a papelera:', error);
    }
  };

  const allVisibleIds = emails.map((email) => email.id);
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.4, mt: 1.5 }}>
        {loading ? (
          ...
          emails.map((email) => {
            ...
            const recipientName = recipient ? (recipient.split('@')[0] || recipient) : 'Destinatario';
